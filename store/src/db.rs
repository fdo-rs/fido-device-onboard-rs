...
{
    match db_type {
        DBType::Postgres => match server_type {
...
{
    async fn load_data(&self, key: &K) -> Result<Option<V>, StoreError> {
        let pool = fdo_db::postgres::PostgresManufacturerDB::get_conn_pool();
        let conn = &mut pool.get().expect("Couldn't establish a connection");
...
{
    async fn load_data(&self, key: &K) -> Result<Option<V>, StoreError> {
        let pool = fdo_db::postgres::PostgresRendezvousDB::get_conn_pool();
        let conn = &mut pool.get().expect("Couldn't establish a connection");
